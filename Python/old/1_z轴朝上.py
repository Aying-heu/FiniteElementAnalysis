...
class UnderwaterVehicle:
    ...
    def update_gEta(self,t):
        ...
        distance_wave_self=wave_height-((self.Eta[2]-self.height/2))  # 波浪高度与AUV底面的距离
    ...
    def simulate(self):
        ...
        for frame in range(int(self.total_frames)):
            ...
            # 旋转顺序：偏航 -> 俯仰 -> 横滚
            # 注意角度转换：
            current_cube.rotate_z(np.degrees(self.psi), inplace=True)  # 偏航角取反
            current_cube.rotate_y(np.degrees(self.theta), inplace=True)   # 横滚
            current_cube.rotate_x(np.degrees(self.phi), inplace=True)  # 俯仰
