...
@cuda.jit
def fast_fsi_kernel(
    init_points,     # [N, 3] 面元中心
    face_normals,    # [N, 3] 面元法向量
    face_areas,      # [N]    面元面积 [关键参数]
    
    rot_matrix,      # [3, 3]
    trans_vec,       # [3]
    g_center,        # [3] 计算力矩的参考点 (通常是物体的几何中心或原点)
    
    wave_k, wave_omega, wave_theta, wave_amp, wave_phi, 
    sim_time,
    
    rho, g, 
    # d_uuv, <--- 删除这个参数，我们使用真实面积
    
    out_force, out_moment
):
    # shared_mem 布局: [TPB, 6]
    ...
    # 初始化共享内存
    if tid < TPB:
        for k in range(6): s_data[tid, k] = 0.0
    ...
    if gid < init_points.shape[0]:
        ...
        cur_x = rot_matrix[0,0]*p0_x + rot_matrix[0,1]*p0_y + rot_matrix[0,2]*p0_z + trans_vec[0]
        cur_y = rot_matrix[1,0]*p0_x + rot_matrix[1,1]*p0_y + rot_matrix[1,2]*p0_z + trans_vec[1]
        cur_z = rot_matrix[2,0]*p0_x + rot_matrix[2,1]*p0_y + rot_matrix[2,2]*p0_z + trans_vec[2]
        
        # 2. 计算波高 eta
        eta = 0.0
        for i in range(wave_k.shape[0]):
            k_val = wave_k[i]
            w_val = wave_omega[i]
        ...
        # 3. 物理计算 (NED坐标系：Z轴向下，Z > eta 表示在水面以下)
        if cur_z > eta: 
            depth = cur_z - eta
            # 压力 P = rho * g * h
            ...
            # 力的大小 = 压力 * 真实面积
            area = face_areas[gid]
            force_mag = pressure * area
            
            # 法向量变换 (Body -> Global)
            n0_x = face_normals[gid, 0]
            n0_y = face_normals[gid, 1]
            n0_z = face_normals[gid, 2]
            ...
            s_data[tid, 1] = fy
            s_data[tid, 2] = fz
            s_data[tid, 3] = ry * fz - rz * fy
            s_data[tid, 4] = rz * fx - rx * fz
            s_data[tid, 5] = rx * fy - ry * fx
    ...
    while stride > 0:
        if tid < stride:
            for k in range(6):
                s_data[tid, k] += s_data[tid + stride, k]
        cuda.syncthreads()
        stride //= 2
...
def create_auv_mesh(length, diameter, n_resolution=50):
    """
    生成 AUV 网格，并确保法线向外
    """
    radius = diameter / 2.0
    ...
    # 1. 创建部件
    # cylinder 默认中心在(0,0,0)，沿X轴
    cylinder = pv.Cylinder(radius=radius, height=cyl_length, 
                           direction=(1, 0, 0), resolution=n_resolution, 
                           capping=False).triangulate()
    
    # 细分以增加面元数量，提高积分精度
    cylinder = cylinder.subdivide(1, subfilter='linear') 

    # 头部球体
    sphere_front = pv.Sphere(radius=radius, center=(cyl_length/2, 0, 0), 
                             theta_resolution=n_resolution, phi_resolution=n_resolution)
    sphere_front = sphere_front.clip(normal='-x', origin=(cyl_length/2, 0, 0), invert=False).triangulate()

    # 尾部球体
    sphere_back = pv.Sphere(radius=radius, center=(-cyl_length/2, 0, 0), 
                            theta_resolution=n_resolution, phi_resolution=n_resolution)
    sphere_back = sphere_back.clip(normal='x', origin=(-cyl_length/2, 0, 0), invert=False).triangulate()

    # 2. 合并
    auv_mesh = cylinder + sphere_front + sphere_back
    auv_mesh = auv_mesh.clean() # 合并重复点
    
    # 3. [关键] 计算法线并强制一致性
    auv_mesh.compute_normals(cell_normals=True, point_normals=False, inplace=True)
    
    # 检查法线方向：对于凸体，法线方向应与 (Center -> FaceCenter) 向量的点积 > 0
    centers = auv_mesh.cell_centers().points
    normals = auv_mesh.cell_normals
    # 假设物体中心大约在 (0,0,0)
    vectors = centers - np.array([0, 0, 0])
    
    # 计算点积
    dots = np.sum(vectors * normals, axis=1)
    
    # 如果大部分点积为负，说明法线反了，需要翻转
    if np.mean(dots) < 0:
        print("检测到法线向内，正在翻转...")
        auv_mesh.flip_normals()
        # 重新计算
        auv_mesh.compute_normals(cell_normals=True, point_normals=False, inplace=True)
...
class UnderwaterVehicle:
    def __init__(self,m,
        ...
        # === 2. 无量纲化系数 (Scaling Factors) ===
        # 根据 SNAME 标准，力 F ~ 0.5*rho*L^2*U^2，力矩 M ~ 0.5*rho*L^3*U^2
        self.L=self.length
        factor_force  = 0.5 * self.rho * (self.L ** 2)  # 用于 u^2 相关的力
        factor_moment = 0.5 * self.rho * (self.L ** 3)  # 用于 u^2 相关的力矩

        # 质量/附加质量 (Xu_dot) ~ 0.5*rho*L^3
        factor_mass   = 0.5 * self.rho * (self.L ** 3)
        # 惯量/附加惯量 (Kp_dot) ~ 0.5*rho*L^5
        factor_inertia= 0.5 * self.rho * (self.L ** 5)
        
        # 混合项推导 (例如 Xwq: Force / (vel * rate) -> Force / (U * U/L) -> Force*L / U^2)
        # 简易对应表:
        scale_mass    = 0.5 * self.rho * (self.L ** 3)  # kg
        scale_mom_ine = 0.5 * self.rho * (self.L ** 5)  # kg*m^2
        scale_force_udot = scale_mass
        scale_mom_wdot   = 0.5 * self.rho * (self.L ** 4) # 也就是 mass * L

        # 阻尼项系数还原
        # Force 相关的项
        Sc_F_uu = 0.5 * self.rho * (self.L ** 2) # Xuu
        Sc_F_wq = 0.5 * self.rho * (self.L ** 3) # Xwq (Force / (m/s * rad/s))
        Sc_F_qq = 0.5 * self.rho * (self.L ** 4) # Xqq (Force / rad/s^2)

        # Moment 相关的项
        Sc_M_uu = 0.5 * self.rho * (self.L ** 3) # Muu
        Sc_M_wq = 0.5 * self.rho * (self.L ** 4) # Mwq
        Sc_M_qq = 0.5 * self.rho * (self.L ** 5) # Mqq

        # === 3. 载入系数并还原为有量纲值 ===
        # --- 附加质量 (Added Mass) ---
        # 对角项
        self.X_udot = -4.874e-3 * scale_mass
        self.Y_vdot = -126.6e-3 * scale_mass   # 注意：C++里 Yv_ 是 -29e-3, Zw_ 是 -126e-3. 
                                                # 通常 Yv_ 对应横向，Zw_ 对应垂向。
                                                # C++代码写了 "z方向... Yv_=..." 看起来像是注释和变量名有些混淆
                                                # 我这里严格按照变量名：
        self.Y_vdot_real = -29.029e-3 * scale_mass # C++: Yv_
        self.Z_wdot = -126.6e-3 * scale_mass   # C++: Zw_
        self.K_pdot = -0.5e-3   * scale_mom_ine # C++: Kp_
        self.M_qdot = -5.043e-3 * scale_mom_ine # C++: Mq_
        self.N_rdot = -1.050e-3 * scale_mom_ine # C++: Nr_

        # 耦合附加质量 (Off-diagonal Added Mass)
        # 根据 M_A 定义，M_ij
        self.Y_rdot = -0.396e-3 * 0.5 * self.rho * (self.L**4) # Yr_ (Force/rad/s^2)
        self.Z_qdot = -1.4e-3   * 0.5 * self.rho * (self.L**4) # Zq_
        self.M_wdot = -1.648e-3 * 0.5 * self.rho * (self.L**4) # Mw_ (Moment/acc)
        self.N_vdot = -0.396e-3 * 0.5 * self.rho * (self.L**4) # Nv_

        # --- 水动力阻尼系数 (Hydrodynamic Coefficients) ---
        # X 方向 (Force)
        self.Xuu = -6.854e-3 * Sc_F_uu
        self.Xwq = -88.246e-3* Sc_F_wq
        self.Xqq = 3.43e-3   * Sc_F_qq
        self.Xvr = 19.42e-3  * Sc_F_wq
        self.Xrr = -2.135e-3 * Sc_F_qq
        self.Xpr = -3.118e-3 * Sc_F_qq # (Force / (rad/s)^2)
        
        # Y 方向 (Force)
        self.Yv   = -41.858e-3 * 0.5 * self.rho * (self.L ** 3) # Linear (normalized by L^3?) 假设 U=1
        self.Yr   = 10.435e-3  * 0.5 * self.rho * (self.L ** 3)
        self.Yv1v1= -28.704e-3 * Sc_F_uu
        self.Yr1r1= 9.189e-3   * Sc_F_qq # Force / r^2
        self.Ypq  = 46.866e-3  * Sc_F_qq
        
        # Z 方向 (Force)
        self.Zw   = -290.9e-3  * 0.5 * self.rho * (self.L ** 3)
        self.Zq   = -145.5e-3  * 0.5 * self.rho * (self.L ** 3)
        self.Zww  = -40.69e-3  * Sc_F_uu
        self.Zqq  = -13.918e-3 * Sc_F_qq # Zq1q1
        self.Zvp  = -31.9e-3   * Sc_F_wq
        self.Zpr  = -0.396e-3  * Sc_F_qq
        self.Zrr  = 1.667e-3   * 0.5 * self.rho * (self.L ** 4)
        
        # K 方向 (Moment)
        self.Kp   = -1.547e-3  * 0.5 * self.rho * (self.L ** 4)
        self.Kr   = -0.041e-3  * 0.5 * self.rho * (self.L ** 4)
        self.Kpp  = -12.47e-3  * Sc_M_qq # Kp1p1
        self.Kpq  = 2.223e-3   * Sc_M_qq
        self.Kqr  = 0.368e-3   * Sc_M_qq
        self.Kwp  = 5.858e-3   * Sc_M_wq
        
        # M 方向 (Moment)
        self.Mw   = 2.621e-3   * 0.5 * self.rho * (self.L ** 3) # Linear W
        self.Mq   = -49.2e-3   * 0.5 * self.rho * (self.L ** 4) # Linear Q
        self.Mww  = -13.329e-3 * Sc_M_uu
        self.Mqq  = -20.427e-3 * Sc_M_qq
        self.Muw  = -7.4e-3    * Sc_M_uu  # Moment / (m/s * m/s) ? Check units. Usually Force coupling.
        self.Muq  = -49.2e-3   * Sc_M_wq
        self.Mvp  = -2.3e-3    * Sc_M_wq
        self.Mpr  = 5.2e-3     * Sc_M_qq
        self.Mrr  = 1.321e-3   * Sc_M_qq
        
        # N 方向 (Moment)
        self.Nv   = -2.872e-3  * 0.5 * self.rho * (self.L ** 3)
        self.Nr   = -7.335e-3  * 0.5 * self.rho * (self.L ** 4)
        self.Nvv  = 17.440e-3  * Sc_M_uu
        self.Nrr  = -6.453e-3  * Sc_M_qq
        self.Npq  = -16.311e-3 * Sc_M_qq
        self.Nwp  = -31.621e-3 * Sc_M_wq
        self.Nvw  = 4.148e-3   * Sc_M_uu

        # === 4. 构建质量矩阵 M ===
        # M_RB (刚体)
        self.M_RB = np.zeros((6, 6))
        self.M_RB[0:3, 0:3] = self.m * np.eye(3)
        self.M_RB[3, 3] = self.Ix
        self.M_RB[4, 4] = self.Iy
        self.M_RB[5, 5] = self.Iz
        S_rg = np.array([
            [0, -self.zg, self.yg],
            [self.zg, 0, -self.xg],
            [-self.yg, self.xg, 0]
        ])
        # 填充耦合项 (Off-diagonal terms)
        # 上右上: -m * S(rg)
        self.M_RB[0:3, 3:6] = -self.m * S_rg
        # 左下角: m * S(rg)
        self.M_RB[3:6, 0:3] = self.m * S_rg
        # 添加重心引起的耦合项 (m * zg * ...)
        # 简化起见，如果重心在几何中心附近，这部分先略过，
        # 或者使用标准公式 M_RB = [mI, -m S(rg); m S(rg), I_g - m S(rg)^2]

        # M_A (附加质量) - 注意符号，通常 M = M_RB - M_A (如果 coeff 定义为力)，
        # 但这里的 coefficients 是 "X_udot"，即 Force = X_udot * acc. 
        # 运动方程: (M_RB - X_udot) * acc = Forces. 
        # 所以总质量矩阵 M_total 的元素是 (m - X_udot).
        # 因为 X_udot 通常是负数，所以 M_total = m + |X_udot|.

        self.M_A = np.zeros((6, 6))
        self.M_A[0, 0] = self.X_udot
        self.M_A[1, 1] = self.Y_vdot_real
        self.M_A[2, 2] = self.Z_wdot
        self.M_A[3, 3] = self.K_pdot
        self.M_A[4, 4] = self.M_qdot
        self.M_A[5, 5] = self.N_rdot
        # 耦合项
        self.M_A[1, 5] = self.Y_rdot
        self.M_A[5, 1] = self.N_vdot
        self.M_A[2, 4] = self.Z_qdot
        self.M_A[4, 2] = self.M_wdot

        # 总质量矩阵 (左边项系数)
        self.M = self.M_RB - self.M_A  # 减去负的附加质量 = 加上正值
       ...
       # 使用新的 create_auv_mesh
        self.auv_mesh = create_auv_mesh(self.length, self.width, n_resolution=60) # 60切分足够
        
        # 计算几何信息
        self.auv_mesh = self.auv_mesh.compute_cell_sizes() # 计算面积
        
        # 获取积分点信息
        cell_centers = self.auv_mesh.cell_centers().points
        cell_normals = self.auv_mesh.cell_normals
        cell_areas = self.auv_mesh.cell_data["Area"] # 获取面积
        
        self.num_cells = cell_centers.shape[0]
        print(f"AUV Mesh 生成完毕: {self.num_cells} 个面元用于积分")
        
        # 传至 GPU
        self.d_points_init = cuda.to_device(cell_centers.astype(np.float32))
        self.d_face_normals = cuda.to_device(cell_normals.astype(np.float32))
        self.d_face_areas = cuda.to_device(cell_areas.astype(np.float32)) # 新增面积
        ...
        self.blocks_per_grid = (self.num_cells + 255) // 256
        ...
        self.plotter.subplot(0, 0)
        # 添加 AUV 模型 (用于显示，这里可以用原始 mesh)
        self.body_actor = self.plotter.add_mesh(self.auv_mesh, color='orange', smooth_shading=True)
        self.plotter.add_axes(line_width=5, labels_off=False) # <--- [添加坐标系]

        self.plotter.subplot(0, 1)
        self.global_body_actor = self.plotter.add_mesh(self.auv_mesh, color='orange', smooth_shading=True)
        self.plotter.add_axes(line_width=5, labels_off=False) # <--- [添加坐标系]
    ...
    #     # 注意：这里 g_center 应该是几何中心（对于 torque 计算参考点）
    #     # 如果你的 self.auv_mesh 是 centered at (0,0,0)，这里就是 (0,0,0)
    #     g_center = np.array(self.auv_mesh.center, dtype=np.float32)
    ...
    #     # 3. 启动核函数
    #     # [关键] 检查这里是否传入了 self.d_face_areas
    #     fast_fsi_kernel[self.blocks_per_grid, self.threads_per_block](
    #         self.d_points_init,   # points
    #         self.d_face_normals,  # normals
    #         self.d_face_areas,    # areas (之前这里可能是 d_uuv，请改过来)
            
    #         cuda.to_device(rot_matrix), cuda.to_device(trans_vec), cuda.to_device(g_center),
    #         self.d_wave_k, self.d_wave_omega, self.d_wave_theta, self.d_wave_amp, self.d_wave_phi,
    #         float32(t),
    #         float32(self.rho), float32(self.g), 
    #         # float32(self.d_uuv), <--- 删除这行
            
    #         self.d_force, self.d_moment
    #     )
    ...
    def update_gEta(self, t):
        ...
        # 计算力矩的几何参考点 (这里设为 0,0,0 即物体坐标系原点)
        # 我们的网格是围绕 (0,0,0) 生成的，所以直接传 0 即可
        ...
        # 3. 启动核函数 (注意去掉了 d_uuv 参数，加入了 face_areas)
        fast_fsi_kernel[self.blocks_per_grid, self.threads_per_block](
            self.d_points_init,   # points
            self.d_face_normals,  # normals
            self.d_face_areas,    # areas
        ...
        # 4. 获取水动力 (Global Frame -> Body Frame)
        total_force_global = self.d_force.copy_to_host()
        total_moment_global = self.d_moment.copy_to_host()
        
        R_inv = rot_matrix.T  # Global to Body
        total_force_body = R_inv @ total_force_global
        total_moment_body = R_inv @ total_moment_global
        ...
        # 在 NED 坐标系中，重力在世界系是 [0, 0, W] (W > 0)
        # 转换到 Body 系: F_g_body = R^T * [0, 0, W]^T
        # 这一列对应旋转矩阵的第三行(或转置后的第三列)
        fg_x = -W * np.sin(self.theta)
        fg_y = W * np.cos(self.theta) * np.sin(self.phi)
        fg_z = W * np.cos(self.theta) * np.cos(self.phi)
        
        F_gravity_body = np.array([fg_x, fg_y, fg_z])
        
        # 计算重力力矩 (相对于物体原点 CO)
        # M_g = r_cg x F_g_body
        r_cg = np.array([self.xg, self.yg, self.zg]) # 重心在物体坐标系的位置
        M_gravity_body = np.cross(r_cg, F_gravity_body)

        # === 6. 汇总恢复力 ===
        g_eta = np.zeros(6)
        
        # 力 = 水动力(浮力) + 重力
        g_eta[0:3] = total_force_body + F_gravity_body
        
        # 力矩 = 水动力矩 + 重力力矩
        g_eta[3:6] = total_moment_body + M_gravity_body
    ...
    def simulate(self,tau_x0,tau_n0):
        ...
        for frame in tqdm(range(int(self.total_frames)),dynamic_ncols=True,mininterval=0.5):
            ...
            rotated_points = (rot_matrix @ self.d_points_init.T).T
            current_points = rotated_points + np.array([self.x, self.y, self.z])
            self.auv_mesh.points = current_points
    ...
    def visualize(self):
        ...
        for frame in pbar:
            ...
            if t in self.global_wave_heights_cache:
                ...
                # 如果你想在 PyVista 里看到波浪拱起（Z向上），需要取反
                # PyVista 默认坐标系也是右手系，Z朝屏幕外/上。
                visual_z = -z_values 
                
                new_points = np.column_stack((
                    self.global_plot_X.ravel(), 
                    self.global_plot_Y.ravel(), 
                    visual_z.ravel() # 使用反转后的 Z 进行渲染
                ))
                ...
                self.water_mesh.point_data["Elevation"] = visual_z.ravel()
            ...
            self.body_actor.user_matrix = transform_mat
            self.global_body_actor.user_matrix = transform_mat
...
鱼雷形，总长度 4米，半径0.3m
h=4-0.3=3.7
体积 pi*r*r*h+(4/3)*pi*r*r*r=1.15866
总浮力=rho*g*V=11650
m<=1187
...
nabla=  m / rho  # 排水体积 m³
GM_L, GM_T = 0.05, 0.05
xg, yg, zg = 0.0, 0.0, 30e-3
xb, yb, zb = 0.0, 0.0, -30e-3
...
if tau_x0==0:
    folder=f"./仿真/无推力-浮力787N-质量{m}kg/"
else:
    folder=f"./仿真/推力{tau_x0}N-浮力787N-质量{m}kg/"
...
csv_name=f"{folder}Hs-{Hs}-T1-{T1}-main_theta-{i}-current_force_mag-{current_force_mag}-current_angle-{current_angle}.csv"
jpg_name=f"{folder}Hs-{Hs}-T1-{T1}-main_theta-{i}-current_force_mag-{current_force_mag}-current_angle-{current_angle}.jpg"
moviepath=f"{folder}Hs-{Hs}-T1-{T1}-main_theta-{i}-current_force_mag-{current_force_mag}-current_angle-{current_angle}.mp4"
