...
@cuda.jit
def compute_pressure_forces_kernel(points, wave_heights, directions, face_ids, 
                                  g_center, rho, g, d_uuv, 
    ...
    if i < points.shape[0]:
        if wave_heights[i] < points[i, 2]:
            # 计算压力
            pressure = rho * g * (points[i, 2] - wave_heights[i]) * d_uuv * d_uuv
            
            # 获取面法向
            face_id = face_ids[i]
            dir_x = directions[face_id, 0]
            dir_y = directions[face_id, 1]
            dir_z = directions[face_id, 2]
            
            # 压力向量
            pressure_vector_x = -pressure * dir_x
            pressure_vector_y = -pressure * dir_y
            pressure_vector_z = -pressure * dir_z
            
            # 位置向量
            r_x = points[i, 0] - g_center[0]
            r_y = points[i, 1] - g_center[1]
            r_z = points[i, 2] - g_center[2]
            
            # 力矩 = r × F
            moment_x = r_y * pressure_vector_z - r_z * pressure_vector_y
            moment_y = r_z * pressure_vector_x - r_x * pressure_vector_z
            moment_z = r_x * pressure_vector_y - r_y * pressure_vector_x
            
            # 原子操作累加
            cuda.atomic.add(total_force, 0, pressure_vector_x)
            cuda.atomic.add(total_force, 1, pressure_vector_y)
            cuda.atomic.add(total_force, 2, pressure_vector_z)
            
            cuda.atomic.add(total_moment, 0, moment_x)
            cuda.atomic.add(total_moment, 1, moment_y)
            cuda.atomic.add(total_moment, 2, moment_z)

def compute_pressure_forces(points, wave_heights_device, directions, face_ids, 
                               g_center, rho, g, d_uuv):
    """
    GPU加速版本的主函数
    """
    ...
    # wave_heights_device = cuda.to_device(wave_heights.astype(np.float32))
    directions_device = cuda.to_device(directions.astype(np.float32))
    face_ids_device = cuda.to_device(face_ids.astype(np.int32))
    g_center_device = cuda.to_device(np.array(g_center, dtype=np.float32))
    
    # 在GPU上分配结果存储
    total_force_device = cuda.device_array(3, dtype=np.float32)
    total_moment_device = cuda.device_array(3, dtype=np.float32)
    
    # 初始化归零
    total_force_device.copy_to_device(np.zeros(3, dtype=np.float32))
    total_moment_device.copy_to_device(np.zeros(3, dtype=np.float32))
    ...
    # 启动核函数
    compute_pressure_forces_kernel[blocks_per_grid, threads_per_block](
        points_device, wave_heights_device, directions_device, face_ids_device,
        g_center_device, np.float32(rho), np.float32(g), np.float32(d_uuv),
        total_force_device, total_moment_device
    )
    ...
    # 将结果传回CPU
    total_force = total_force_device.copy_to_host()
    total_moment = total_moment_device.copy_to_host()
...
class UnderwaterVehicle:
    ...
    def update_gEta(self, t):
        ...
        face_centers = np.mean(vertices[self.faces], axis=1) 
        directions = g_center - face_centers
        ...
        for i, face in enumerate(self.faces):
            ...
            # 计算面的尺寸
            len_u = np.linalg.norm(basis_u)
            len_v = np.linalg.norm(basis_v)
            
            # 计算采样点数量
            num_u = max(1, int(len_u / self.d_uuv))
            num_v = max(1, int(len_v / self.d_uuv))
            
            # 创建网格
            u_grid, v_grid = np.meshgrid(
                np.linspace(0.5, len_u - 0.5, num_u) / len_u,
                np.linspace(0.5, len_v - 0.5, num_v) / len_v
            )
            ...
            # 向量化计算所有点
            points = v0 + np.outer(u_flat, basis_u) + np.outer(v_flat, basis_v)
            
            all_points_list.append(points)
            face_ids_list.append(np.full(len(points), i))
        
        # 合并所有结果
        all_points = np.vstack(all_points_list)
        face_ids = np.concatenate(face_ids_list)
        

        # 一次性计算所有点的波高
        # 计算所有点的波高
        wave_heights_gpu = self.wave_elevation_points(all_points, t)
        
        directions_arr = np.array(directions)
        total_force_global, total_moment_global = compute_pressure_forces(
            all_points, wave_heights_gpu, directions_arr, face_ids, g_center, 
    ...
    def simulate(self,tau_x0,tau_n0):
        ...
        for frame in tqdm(range(int(self.total_frames)),dynamic_ncols=True):
...
for i in range(0,360,20):
    ...
    csv_name=f"./Hs-{Hs}-T1-{T1}-main_theta-{i}-current_force_mag-{current_force_mag}-current_angle-{current_angle}.csv"
    jpg_name=f"./Hs-{Hs}-T1-{T1}-main_theta-{i}-current_force_mag-{current_force_mag}-current_angle-{current_angle}.jpg"
