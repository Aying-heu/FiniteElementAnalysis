...
__global__ void compute_face_force_kernel(
    ...
    for (int w = 0; w < num_waves; w++) {
        double A = amp[w];
        double w_val = omega[w];
        double k_val = k_arr[w];
        double eps = phase[w];
        double th = theta[w];

        double phase_val = k_val * (center_x * cos(th) + center_y * sin(th)) - w_val * t + eps;
        double cos_val = cos(phase_val);

        eta += A * cos_val;

        // Smith 效应动压力
        double decay_z = center_z;
        // 保护：如果飞得太高 (例如 2米以上)，强行截断，避免 exp 溢出
        if (decay_z < -5.0) decay_z = -5.0; 

        double decay = exp(-k_val * decay_z);
        // 再次保护结果
        if (decay > 20.0) decay = 20.0;
