...
@njit(parallel=True)
def compute_hydrostatic_pressures(points, wave_heights, directions, face_ids, g_center, rho, g, d_uuv):
...
# @njit(parallel=True)
# def compute_dynamic_pressure(points, wave_heights, directions, face_ids, g_center, rho, g, d_uuv,t,
#                              amplitudes,omega,wave_theta,k,wave_N,wave_M,phi_ij,h):
#     total_force = np.zeros(3)
#     total_moment = np.zeros(3)

#     X = points[:,0]  # 无需额外转np.array（假设points已是numpy数组）
#     Y = points[:,1]
#     Z = points[:,2] - wave_heights
#     # Z = points[:,2]
#     Z = np.where(Z > 0, Z, 0) 

#     for face_id in prange(len(points)):
#         if Z[face_id]<=0:
#             continue
#         pressure=0.0
...
#                 phases=k_i*(X[face_id]* np.cos(theta_j) + Y[face_id] * np.sin(theta_j)) - omega_i * t + phi_ij[i, j]
#                 # 计算深度衰减因子
#                 if h < 1e10:  # 有限水深
#                     depth_factor = np.cosh(k_i * (Z[face_id] + h)) / np.cosh(k_i * h)
#                 else:  # 无限水深近似
#                     depth_factor = np.exp(k_i * Z[face_id])  # Z为负值，保证衰减
#                 pressure+=rho*g*amplitudes[i,j]*depth_factor*np.cos(phases)
#         # print(f"pressure={pressure}")
#         pressure_vector = -pressure * d_uuv**2 * directions[face_ids[face_id]]
#         r = points[face_id] - g_center
#         moment = np.cross(r, pressure_vector)
...
def compute_dynamic_pressure(points, wave_heights, directions, face_ids, g_center, rho, g, d_uuv, t,
    ...
    for face_id in prange(len(points)):
        # 计算水面以下深度（z轴向下为正）
        z_depth = points[face_id, 2] - wave_heights[face_id]
        ...
        for i in range(len(omega)):
            ...
            for j in range(len(wave_theta)):
                ...
                if h < 1e5:  # 有限水深
                    ...
                    # 当水深很大时使用无限水深近似
                    if kh > 50:
                        depth_factor = np.exp(-k_i * z_val)
                    else:
                        # 正确的有限水深公式
                        depth_factor = np.cosh(k_i * (h - z_val)) / np.cosh(kh)
                else:  # 无限水深
                    depth_factor = np.exp(-k_i * z_val)
                
                # 累加压力分量
                pressure_component = rho * g * amplitudes[i, j] * depth_factor * np.cos(phase)
                
                # 确保不是NaN
                if not np.isnan(pressure_component):
                    pressure += pressure_component
        
        # 计算压力向量（指向物体内部）
        pressure_force = pressure * area
        # 修正索引：使用face_id而不是i
        pressure_vector = -pressure_force *directions[face_ids[face_id]]
...
class UnderwaterVehicle:
    ...
    def update_gEta_and_tau_wave(self, t):
        ...
        total_force_global, total_moment_global = compute_hydrostatic_pressures(
        ...
        total_force_global_dynamic = np.zeros(3)
        total_moment_global_dynamic = np.zeros(3)


# compute_dynamic_pressure(points, wave_heights, directions, face_ids, g_center, rho, g, d_uuv,t,
#                              amplitudes,omega,wave_theta,k,wave_N,wave_M,phi_ij,h):
# def compute_dynamic_pressure(points, directions, face_ids, g_center, rho, g, d_uuv, t,
#                              amplitudes, omega, wave_theta, k, wave_N, wave_M, phi_ij, h):
        total_force_global_dynamic,total_moment_global_dynamic=compute_dynamic_pressure(
            all_points,wave_heights, directions_arr, face_ids, g_center, 
            self.rho, self.g, self.d_uuv,t,
            self.amplitudes,self.omega,self.wave_theta,self.k,self.wave_N,self.wave_M,self.phi_ij,3000
        )

        total_force_body_dynamic = R_inv @ total_force_global_dynamic
        total_moment_body_dynamic = R_inv @ total_moment_global_dynamic
        

        Tau_wave = np.zeros(6)
        Tau_wave[0:3] = total_force_body_dynamic
        
        # 恢复力矩 = 浮力矩(物体坐标系)
        Tau_wave[3:6] = total_moment_body_dynamic
        ...
        # print(f"t={t}")
        # print(f"g_eta={g_eta}")
        # print(f"total_force_global_dynamic={total_force_global_dynamic}")
        # print(f"total_moment_global_dynamic={total_moment_global_dynamic}")
        # print(f"total_force_body_dynamic={total_force_body_dynamic}")
        # print(f"total_moment_body_dynamic={total_moment_body_dynamic}")
        # print(f"Tau_wave={Tau_wave}")
    # def update_gEta(self, t):
    #     total_force_global = np.zeros(3)  # 全局坐标系中的总力
    #     total_moment_global = np.zeros(3)  # 全局坐标系中的总力矩（关于重心）
    ...
    def update_params(self,t):
        ...
        self.update_CV()
        self.update_DV()
        self.update_gEta_and_tau_wave(t)
    ...
    def simulate(self):
        ...
        for frame in tqdm(range(int(self.total_frames))):
            ...
            self.V_dot = np.linalg.inv(self.M) @ (self.Tau + self.Tau_wave - self.C_V @ self.V - self.D_V @ self.V - self.g_Eta)
